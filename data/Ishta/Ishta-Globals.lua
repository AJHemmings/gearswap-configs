-- Place to set a universal lock style, set to empty if you want per-job
global_lockstyle = '12'
...
send_command('bind ^f1 gs c toggle AutoRemoveDoomMode')
send_command('exec init')
--send_command('bind !a sa input /assist Tegamen;wait 0.5;input /attack on')

--send_command('exec binds')

NotifyBuffs = {'doom','petrification'}
...
function user_self_command(commandArgs, eventArgs)
	...
	if cmd == 'TakePhoto' then
		windower.add_to_chat(8,'Taking photo...')
		send_command('input /item "Soultrapper 2000" <bt>')
	end
end