...
--State for Trump Card check
state.trump_warned = M(false)
...
-- Threshold for Trump Card (Quick Draw) low warning
Trump_Card_Warning_Limit = 10
...
do
	...
	-------------------------------------------------------------------------------------------------------------------
	-- Determine whether we have sufficient Trump Cards for Quick Draw, warn when low, and restock from a Trump Card Case.
	-------------------------------------------------------------------------------------------------------------------

	function do_trump_card_checks(spell)
		if spell.type ~= 'CorsairShot' then return end

		local available_cards = player.inventory['Trump Card'] and player.inventory['Trump Card'].count or 0
		log('Trump Card Count ['..available_cards..']')

		if available_cards == 0 then
			-- Out entirely: cancel this Quick Draw (nothing to consume) and try to restock immediately.
			add_to_chat(104, 'Out of Trump Cards.  Attempting to restock.')
			restock_trump_cards(0)
			cancel_spell()
			return
		end

		if state.trump_warned.value == false and available_cards <= Trump_Card_Warning_Limit then
			local msg = '*****  LOW TRUMP CARD WARNING: '..tostring(available_cards)..'x Trump Card *****'
			local border = ""
			for i = 2, #msg do border = border .. "*" end
			windower.send_command('send @others input /echo '..msg..'')
			add_to_chat(167, border)
			add_to_chat(167, msg)
			add_to_chat(167, border)
			-- Quick Draw is still going through this time, so delay the item use
			-- so it doesn't race with the Quick Draw action currently being sent.
			restock_trump_cards(2)
			state.trump_warned:set()
		elseif available_cards > Trump_Card_Warning_Limit and state.trump_warned then
			state.trump_warned:reset()
		end
	end

	-------------------------------------------------------------------------------------------------------------------
	-- Uses a Trump Card Case to restock Trump Cards, pulling the case from Satchel/Sack/Case if it's not
	-- already in main inventory. 'delay' (seconds) postpones the actual item use, e.g. to avoid overlapping
	-- with an action already in flight; 0 uses it immediately.
	-------------------------------------------------------------------------------------------------------------------

	function restock_trump_cards(delay)
		delay = delay or 0

		if player.inventory['Trump Card Case'] then
			if delay > 0 then
				windower.chat.input:schedule(delay,'/item "Trump Card Case" <me>')
			else
		...
		elseif player.satchel['Trump Card Case'] then
			...
			windower.chat.input:schedule(math.max(delay,1.5),'/item "Trump Card Case" <me>')
		elseif player.sack['Trump Card Case'] then
			windower.send_command('get "Trump Card Case" sack')
			windower.chat.input:schedule(math.max(delay,1.5),'/item "Trump Card Case" <me>')
		elseif player.case['Trump Card Case'] then
			windower.send_command('get "Trump Card Case" case')
			windower.chat.input:schedule(math.max(delay,1.5),'/item "Trump Card Case" <me>')
		else
			add_to_chat(167, 'No Trump Card Case found in Inventory, Satchel, Sack, or Case.')
		end
	end
